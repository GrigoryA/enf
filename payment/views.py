...
def create_heleket_payment(order, request):
    ...
    encoded_payload = base64.b64encode(
        payload_str.encode('utf-8')).decode('utf-8')
    sign = hashlib.md5(
        (encoded_payload + HELEKET_SECRET_KEY).encode('utf-8')).hexdigest()
    ...
    try:
        response = requests.post(
            f"{HELEKET_BASE_URL}/payment", headers=headers, data=payload_str)
        if response.status_code == 200:
            payment = response.json()
            if payment.get('state') == 0:
                order.heleket_payment_id = payment.get(
                    'result', {}).get('uuid')
                ...
                raise Exception(
                    f'Error creating Heleket payment: response={response.text}')
        else:
